...
class OrderEntrustBase(Common, db.Model):
    ...
    driver_uuid = db.Column(db.String(length=32), db.ForeignKey('driver.uuid'), nullable=False, index=True,
                            comment='驾驶员UUID')
...
class DriverOrderScheduleLogBase(Common, db.Model):
    ...
    driver_order_uuid = db.Column(db.String(24), db.ForeignKey('driver_order.order_uuid'), comment='订单编号')
